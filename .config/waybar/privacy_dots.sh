...
# Add empty class and alt when all indicators are off
alt=""
if [[ $mic -eq 0 && $cam -eq 0 && $loc -eq 0 && $scr -eq 0 ]]; then
  classes="$classes empty"
  alt="privacydot-empty"
fi

jq -c -n --arg text "$text" --arg tooltip "$tooltip" --arg class "$classes" --arg alt "$alt" \
